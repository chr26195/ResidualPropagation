import torch
import os
from torch_geometric.datasets import Planetoid, HeterophilousGraphDataset, GitHub, FacebookPagePage, Actor, WebKB, WikipediaNetwork
...
def load_dataset(data_dir, dataset_name, runs, train_ratio = None, fixed_split = False):
    ...
    # from paper 'Multi-Scale Attributed Node Embedding'
    elif dataset_name in ('github'):
        dataset = GitHub(root=os.path.join(data_dir, 'GitHub'))
        data = dataset[0]
        n = data.y.shape[0]
        data = T.RandomNodeSplit('train_rest', num_val=int(n*0.04), num_test=int(n*0.92), num_splits=runs)(data) # 4% for training
        data.metric = "acc"
    elif dataset_name in ('facebook'):
        dataset = FacebookPagePage(root=os.path.join(data_dir, 'Facebook'))
        data = dataset[0]
        n = data.y.shape[0]
        data = T.RandomNodeSplit('train_rest', num_val=int(n*0.04), num_test=int(n*0.92), num_splits=runs)(data) # 4% for training
        data.metric = "acc"
    ...
    # from paper 'Are we really making much progress? Revisiting, benchmarking, and refining heterogeneous graph neural networks'
    # elif dataset_name in ('acm', 'dblp', 'imdb', 'freebase'):
    #     dataset = HGBDataset(root=os.path.join(data_dir, 'HGBDataset'), name = dataset_name)
    #     data = dataset[0].to_homogeneous()
    #     data.x = F.one_hot(data.node_type).float()  # node feature is one-hot embedding of node-type
    #     data.metric = "f1"

    # from paper 'Deep Gaussian Embedding of Graphs: Unsupervised Inductive Learning via Ranking'
    # elif dataset_name in ('dblp'):
    #     dataset = CitationFull(root=os.path.join(data_dir, 'CitationFull'), name = 'dblp')
    #     data = dataset[0]
    #     n = data.y.shape[0]
    #     data = T.RandomNodeSplit('train_rest', num_val=int(n*0.1), num_test=int(n*0.8), num_splits=runs)(data) # 10% for training, follows https://github.com/CRIPAC-DIG/GRACE
    #     data.metric = "acc"
